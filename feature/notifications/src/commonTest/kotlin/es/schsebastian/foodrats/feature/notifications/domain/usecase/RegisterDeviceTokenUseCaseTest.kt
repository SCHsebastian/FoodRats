...
class RegisterDeviceTokenUseCaseTest {
    ...
    @Test fun upserts_token_when_signed_in() = runTest {
        val repo = FakeRepo()
        val provider = FakeTokenProvider(DeviceToken("t-1"))
        val uc = RegisterDeviceTokenUseCase(provider, repo, FakeSession(Session(me, null)))
        val r = uc()
    ...
    @Test fun returns_Token_Unavailable_when_provider_has_no_token() = runTest {
        val repo = FakeRepo()
        val uc = RegisterDeviceTokenUseCase(FakeTokenProvider(null), repo, FakeSession(Session(me, null)))
    ...
    @Test fun returns_Token_Unavailable_when_not_signed_in() = runTest {
        val repo = FakeRepo()
        val uc = RegisterDeviceTokenUseCase(FakeTokenProvider(DeviceToken("t")), repo, FakeSession(null))
        assertEquals(Result.failure(NotificationError.Token.Unavailable), uc())
    }
    ...
    @Test fun propagates_repo_error() = runTest {
        ...
        val uc = RegisterDeviceTokenUseCase(FakeTokenProvider(DeviceToken("t")), repo, FakeSession(Session(me, null)))
