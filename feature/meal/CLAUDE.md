...
The rich-domain exemplar: native camera + system photo picker (in-house `rememberPhotoPicker`, `:core:presentation`) → compose draft → publish a `Meal`. Owns the `MealReadPort` / `MealRatingPort` implementations consumed by `:feature:feed` and `:feature:stats`.
