...
private class FakeSessionForRemoveBanner(private val session: Session?) : SessionProvider {
...
class RemoveCrewBannerUseCaseTest {
    ...
    private val sampleCrew = Crew.of(
        ...
        bannerPath = "crew_banners/c-1/banner.jpg",
    )

    @Test fun owner_can_remove_banner() = runTest {
        val session = Session(accountId = ownerId, activeCrewId = crewId)
        val repo = FakeCrewRepository(listOf(sampleCrew))
        val useCase = RemoveCrewBannerUseCase(repo, FakeSessionForRemoveBanner(session))
        val r = useCase(crewId)
        assertIs<Result.Ok<Unit>>(r)
        assertNull(repo.crews.value.first { it.id == crewId }.bannerPath)
    }
    ...
    @Test fun no_session_returns_unavailable() = runTest {
        val repo = FakeCrewRepository(listOf(sampleCrew))
        val useCase = RemoveCrewBannerUseCase(repo, FakeSessionForRemoveBanner(null))
        val r = useCase(crewId)
