...
/**
 * Removes the crew hero/banner image (C9). Delegates to [CrewRepository.removeBanner] which enforces
 * owner-only authorization, deletes the Storage object (NOT_FOUND-tolerant), then clears the
 * Firestore `bannerPath` field.
 */
class RemoveCrewBannerUseCase(
    ...
    suspend operator fun invoke(crewId: CrewId): Result<Unit, CrewError> {
        ...
        return repository.removeBanner(crewId, accountId)
    }
}
