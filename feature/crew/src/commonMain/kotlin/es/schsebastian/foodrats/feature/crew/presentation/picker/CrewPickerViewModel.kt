...
class CrewPickerViewModel(
    ...
    private suspend fun doCreate() {
        ...
        when (val r = createCrew(state.createInput, account, /* TODO displayName from profile */ "")) {
            is Result.Ok  -> {
    ...
    private suspend fun doJoin() {
        ...
        when (val r = joinCrew(state.joinInput, account, /* TODO displayName from profile */ "")) {
            is Result.Ok  -> {
