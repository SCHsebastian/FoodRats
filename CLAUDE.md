...
- **`CrewSettingsViewModelTest` is minimal:** currently covers only the Remove-member flow + owner-vs-member computation. The pre-Profile-split test that exhaustively covered the my-profile rename/avatar/sign-out flows was deleted (those flows moved to `ProfileViewModel`). Expand coverage when touching crew-name save / leave / delete / Switch-crew.
