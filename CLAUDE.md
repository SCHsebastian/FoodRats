...
- Firestore Security Rules: `firestore.rules` + `firebase.json` at the repo root. Deploy with `firebase deploy --only firestore:rules --project <project-id>` (requires interactive `firebase login`; no CI hookup).
...
### Design system v2 — healthy refresh (commit `986b95b`)

- **What.** New palette in `core/designsystem/theme/Colors.kt`: avocado primary `#3FB950` / honey-citrus secondary `#FFB020` / berry tertiary `#E84B6A`, on warm-cream surfaces (`#FBFAF6`) in light and forest-night (`#0F1411`) in dark. Both `lightColorScheme` and `darkColorScheme` are now fully populated (the previous dark scheme reused the light primary). New `FrSemanticColors` data class + `LocalFrSemanticColors` composition local in `theme/SemanticColors.kt` for meaning roles. Typography ramp expanded to 15 styles with `title*`/`label*` weights bumped to 600; `FrTextStyles.statNumber` exposes tabular numerals (`fontFeatureSettings = "tnum"`) for streaks/scores/leaderboards. Radius scale shifted softer (`sm=12`, `md=16`, `lg=24`, `xl=32`). Elevation gained an M3 `level0..level5` ramp; legacy `none/sm/md/lg` aliases preserved. `Sizes` added `streakBadge`, `mealCardImage`, `bottomBarHeight`, `touchTarget`.
- **Why.** The old tomato-red palette read "fast-food brand," not "healthy meal-sharing." Dark theme wasn't real (same primary as light). Atoms had no semantic tokens, so every feature had to either reuse Material brand roles for meaning or hardcode hex. Tabular numerals matter because streak counts and scores live in tight cards and would otherwise jitter on update.
- **How.** All atoms compose against `MaterialTheme.colorScheme` / `typography` / `shapes`, so the swap propagates without touching feature code. For meaning-named color (success/danger/warning/celebration/streakHot), use `LocalFrSemanticColors.current` — never raw `Color(0x…)`, never alias `colorScheme.secondary` for "warning" just because both are yellow. For numeric displays where width stability matters, use `FrTextStyles.statNumber` or `statNumberSmall`. The spec lives at `docs/specs/2026-05-19-healthy-design-system-design.md`. Plus Jakarta Sans is the target font family — `FrFontFamily` falls back to `FontFamily.SansSerif` until font assets are dropped into `composeResources/font/`; then flip the one constant.
