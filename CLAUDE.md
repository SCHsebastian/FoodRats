...
- **iOS: link `CoreLocation.framework` manually in Xcode.** ImagePickerKMP 1.0.41 references `CLLocation` symbols unconditionally in its iOS EXIF extractor (no opt-out). After the next clean iOS build, if the linker reports `_OBJC_CLASS_$_CLLocation`, open `iosApp/iosApp.xcodeproj` → target `iosApp` → Build Phases → "Link Binary With Libraries" → `+` → add `CoreLocation.framework`. There is no Gradle-side fix.
- **Meal-feed enrichment still reads the legacy `crews/{id}.members` cache.** `FirebaseMealRepository` enriches each meal with its author's display name + avatar via `CrewMembersPort.observeMembers`, which reads the denormalized fields seeded at `createCrew` / `joinByCode` time. After Phase D those fields are no longer updated, so author names shown in the feed will go stale after a profile rename until the next crew lifecycle event. Migrate `:feature:meal` to live `AccountReadPort.observeMany` (mirroring `CrewSettings`) to remove the last consumer of the cache; that lets `MemberDto.displayName/avatarUrl` and the `crews/{id}.members.{uid}.displayName/avatarUrl` Firestore fields be dropped entirely.
