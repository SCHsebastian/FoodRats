...
### Meal AI ingredient classification — Phase 5 done, NOT yet wired/runnable (2026-05-26, branch `feat/meal-ai-ingredient-classification`)
...
- **How.** `:feature:meal` must **never** import `:feature:meal-ai` or `:feature:ingredient` (cross-feature ban). The composer classifies via a **meal-owned** `ClassifyDraftPlateUseCase` over the two `:core:domain` ports — *not* meal-ai's `ClassifyPlateUseCase` (the plan's T24 said to inject that; it's wrong — spec §7.3/§408 is authoritative). Classification is **advisory**: failures show a banner, never gate `canContinue`/publish. The VM dedupes by photo `contentHashCode()` (re-entry no-op, re-capture overwrites). **Status: code-complete through Phase 5 (T18–T28) and `assembleDebug` is green, but the feature does NOT run yet** — `:feature:ingredient` has no Koin module and `:feature:meal-ai` only has an iOS one, so `MealClassifierPort`/`IngredientReadPort`/`SelectIngredientsViewModel` are unbound (crashes on navigate). That wiring is T30 (the gating task), then T29 + T31. **Phase 5 (committed `227d42a`→`7f98691`):** seed JSON (`functions/seed/ingredients.json` = 226 ingredients en/es; `functions/seed/dish-ingredient-map.json` = 101 Food-101 classes, full referential integrity), `functions/scripts/seed-catalog.ts` admin uploader + `functions/__tests__/seed.test.ts` (vitest, 9/9 — added `vitest`+`tsx` devDeps and `esbuild: true` in `functions/pnpm-workspace.yaml`), and `firestore.rules` `ingredients`/`dishIngredientMap` blocks (public read, no client write). **Two prod-touching Phase-5 steps still need the user** (write to `foodrats-de4ec`): deploy rules (`pnpm dlx firebase-tools deploy --only firestore:rules --project foodrats-de4ec`) and run the seed (`GOOGLE_APPLICATION_CREDENTIALS=… pnpm --dir functions seed:catalog`) — until both run the picker catalog is empty. See `docs/superpowers/plans/2026-05-24-meal-ai-ingredient-classification.md`.
